...
async function main() {
    ...
    const totalYUSDOCT20Amount = await YUSD_USDC_BALANCER_POOL.getBalance(YUSDOCT20_TOKEN_ADDR) / 1e18;
    const totalUSDCAmount = await YUSD_USDC_BALANCER_POOL.getBalance(USDC_TOKEN_ADDR) / 1e6;


    const YUSDOCT20PerBPT = totalYUSDOCT20Amount / totalBPTAmount;
    ...
    const YUSDOCT20Price = (await YUSD_USDC_BALANCER_POOL.getSpotPrice(USDC_TOKEN_ADDR, YUSDOCT20_TOKEN_ADDR) /1e6) * USDCPrice;

    const BPTPrice = YUSDOCT20PerBPT * YUSDOCT20Price + USDCPerBPT * USDCPrice;

    // Finished. Start printing

    _print("========== PRICES ==========")
    _print(`1 UMA         = $${UMAPrice}`);
    _print(`1 yUSD-OCT20  = $${YUSDOCT20Price}`);
    _print(`1 USDC        = $${USDCPrice}\n`);
    _print(`1 BPT         = [${YUSDOCT20PerBPT} yUSD-SEP20, ${USDCPerBPT} USDC]`);
    ...
    _print(`                  = [${YUSDOCT20PerBPT * yourBPTAmount} yUSD, ${USDCPerBPT * yourBPTAmount} USDC]`);
